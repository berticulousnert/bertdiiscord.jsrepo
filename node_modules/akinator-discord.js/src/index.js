...
module.exports = async function (input, options = {}) {
  ...
  const yesClickedButton = new Discord.MessageButton()
	.setCustomId('iwdidjsksjf')
	.setLabel('✅ Yes')
	.setStyle('SUCCESS')
	.setDisabled(true);
  const noClickedButton = new Discord.MessageButton()
	.setCustomId('iwdijdjddjsjf')
	.setLabel('❌ No')
	.setStyle('SECONDARY')
	.setDisabled(true);  

		const whatDidYaPick = new Discord.MessageActionRow()
			.addComponents(yesClickedButton)
      .addComponents(noClickedButton)
...
let finishedGameCorrect = new Discord.MessageEmbed()
                                ...
                                .addField(translations.noOfQuestions, `**${aki.currentStep}**`, true)
                                .setColor("RANDOM")
                            if (options.useButtons) {await response.update({ embeds: [guessEmbed], components: [whatDidYaPick] }) 
                              input.channel.send({embeds: [finishedGameCorrect], components: []})                             
notFinished = false;
                            games.delete(inputData.author.id)
                            return;                              } else { await akiMessage.edit({ embeds: [guessEmbed], components: [whatDidYaPick] })
                              input.channel.send({embeds : [finishedGameCorrect], components: []})
